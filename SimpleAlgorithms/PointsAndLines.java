...
class LineList implements ListOfLines {
  ...
  @Override
  public Vector<Line> getLinesFromStartingPoint(Point p) {
    Vector<Line> linesFromStartingPoint = new Vector<>();
    lines.forEach(line -> {
      Point point = line.getStart();
      if(point.getX() == p.getX() && point.getY() == p.getY()){
        linesFromStartingPoint.add(line);
      }
    });
    return linesFromStartingPoint;
  }

  @Override
  public Line getLineWithMaxLength() {
    Line maxLengthLine = lines.get(0);
    for (Line line: lines) {
      if(line.getLineLength() > maxLengthLine.getLineLength()) maxLengthLine = line;
    }
